    ...
    try:
        ...
        for item in result:
            if item[0] is not None:
                pickCode = item[0]
                # print(pickCode)
                sql = "SELECT `pick_area`, `group` FROM wcs.pick_areas WHERE code = " + \
                    "'" + str(pickCode) + "'"
                cursor.execute(sql)
                ...
                if result is not None and result[0] is not None:
                    print(result[0])
                    print(result[1])
                    sql = "UPDATE assignment.dat_master SET pick_area="+"'" + \
                        result[0] + "', pick_group="+"'" + result[1] + \
                        "' WHERE pick_code="+"'" + item[0] + "'"
                    cursor.execute(sql)
                    connection.commit()
                    counter += 1
