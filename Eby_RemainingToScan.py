...
def remaining_to_scan(route, stop):
    ...
    for r in records:
        pick_code = "SELECT pick_code FROM assignment.dat_master WHERE id=" + "'" + str(r[0]) + "'"
        cursor.execute(pick_code)
        pick_code = cursor.fetchone()[0]
        print(pick_code)
        
        flag = "SELECT count_flag FROM wcs.pick_areas WHERE code=" + "'" + str(pick_code) + "'"
        cursor.execute(flag)
        ...
        if flag == "1":
            cursor.execute("UPDATE assignment.dat_master SET count_flag=1 WHERE id=" +str(r[0])+";")
