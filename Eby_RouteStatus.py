...
def route_status():
    ...
    for item in result:
        ...
        if dashboard_map_detail['is_all_dashboard_map_one'] == True:
            ...
            cursor.execute("UPDATE wcs.dashboard_routes" + str(item[3]) + " SET door_no_read=0 WHERE route_type = 'current';")
            cursor.execute("UPDATE wcs.dashboard_stops" + str(item[3]) + " SET door_no_read=0 WHERE stop_type = 'current';")
            cursor.execute("UPDATE wcs.dashboard_stops" + str(item[3]) + " SET door_no_read=0 WHERE stop_type = 'previous';")             
            connection.commit()
...
def get_dashboard_map_detail(route, date):
    sql = "SELECT dashboard_map FROM assignment.dat_master WHERE route_no='"+ str(route) +"' AND date='"+ str(date) +"'"
    cursor.execute(sql)
    result = cursor.fetchall()
    total = len(result)
    zero_values = [
